...
How herdr-web works, and what measuring herdr's API taught us along the way.
The [README](../README.md) covers running it; this is everything else.
...
log (see [Security](../README.md#security)) — payload content is
...
- **Browser notifications**: the bell button in the home header asks for
  permission and toggles delivery; it is hidden when the browser has no
  `Notification` API and swaps to `bell-off` when permission was denied.
...
the tail moves past the loaded block. Switching the ANSI toggle or opening another
...
library. The palette button in the detail header toggles it (`aria-pressed`,
